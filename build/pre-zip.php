...
if ( $edition == 'lite' ) {
	...
	if ( 'Y' == trim( fgets( STDIN ) ) ) {
		echo `svn co -q http://svn.wp-plugins.org/help-scout svn`;
		echo `rm -R svn/trunk`;
	...
	if ( 'Y' == trim( fgets( STDIN ) ) ) {
		system( 'git clone git@github.com:SproutApps/help-scout github1' );
		system( 'mkdir github' );
	...
	echo "Replacing the readme.txt within dropbox.\n";
	system( "cp $run_dir/readme-pro.txt /Users/dancameron/Dropbox/Public/help-scout-readme.txt" );
	system( "$build_dir/build-addons" );
}
