...
<form action="" method="post" enctype="multipart/form-data" id="hsd_message_form" class="form" role="form">
	...
	<?php if ( ! $conversation_view ) : ?>
		...
		<div class="form-group">
			...
			<input type="text" class="form-control" id="hsd_subject" name="subject" placeholder="<?php _e( 'How can we help?' , 'help-scout-desk' ) ?>" required="required">
		</div>
	<?php endif ?>
	...
	<div class="form-group">
		...
		<textarea name="message" class="form-control" id="hsd_message" rows="10" placeholder="<?php _e( 'Please include any information that you think will help us generate a speedy response.' , 'help-scout-desk' ) ?>" required="required" ></textarea>
		<?php if ( $conversation_view ) : ?>
